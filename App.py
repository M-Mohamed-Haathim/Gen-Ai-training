...
with st.sidebar:
    ...
    # Prefer a key set in Streamlit Cloud secrets; otherwise let the user paste one
    default_key = st.secrets.get("GROQ_API_KEY", "") if hasattr(st, "secrets") else ""
    api_key = st.text_input(
        "Groq API Key",
        value=default_key,
        type="password",
        help="Set this permanently via Streamlit Cloud → App settings → Secrets as GROQ_API_KEY.",
    )
